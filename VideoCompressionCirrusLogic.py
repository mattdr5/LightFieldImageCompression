...
def comp_cljr(input_path, output_path):          #Support only Lossy compression
    ...
    # Verifica il sistema operativo e imposta il percorso all'eseguibile FFmpeg di conseguenza
    ...
    # Chiama FFmpeg per comprimere il video con il codec Cirrus Logic AccuPak (cljr)
    subprocess.run([ffmpeg_executable, "-framerate", "120", "-i", input_file, "-c:v", "cljr", output_file])
